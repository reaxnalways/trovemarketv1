...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (category.slug === "teknik-servis") {
    ...
    return <><SiteHeader settings={settings} /><main className="shell categoryPageShell"><Link className="backLink" href="/">← Ana sayfaya dön</Link><section className="categoryHero"><p className="eyebrow">{settings.site_name.toUpperCase()} TEKNİK SERVİS</p><h1>{category.name}</h1><p className="heroText">Servis türünü seç; hazırlanmış mesajla doğrudan Trove Teknoloji WhatsApp hattına geç.</p></section><section className="categories homeCategories" aria-label="Teknik servis türleri">{services.map(([label, subject]) => <a className="card categoryLink" href={whatsappHref(settings.whatsapp_number, `Merhaba Trove Teknoloji, ${subject} hakkında bilgi almak istiyorum.`)} key={label} rel="noreferrer" target="_blank"><div><h2>{label}</h2><p>Hızlı bilgi ve servis yönlendirmesi al.</p></div><span className="categoryAction">WhatsApp →</span></a>)}</section></main></>;
  }
