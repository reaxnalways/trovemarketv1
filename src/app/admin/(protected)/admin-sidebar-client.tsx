...
const navGroups = [
  { label:"Ürün Yönetimi", items:[{label:"Ürünler",href:"/admin/listings"},{label:"Yeni ürün ekle",href:"/admin/listings/new"},{label:"Kategoriler",href:"/admin/categories"},{label:"Fiyat Yönetimi",href:"/admin/pricing"},{label:"Siparişler",href:"/admin/purchases"},{label:"Barkod tara",href:"/admin/scan"},{label:"Etiket yazdır",href:"/admin/labels"}] },
...
export function AdminSidebarClient({ siteName, logoUrl, wordmarkUrl }: Props) {
  ...
  const brand = (
    <Link href="/admin" className="adminSidebarBrandLink" onClick={() => setOpen(false)}>
      {logoUrl ? <img className="adminSidebarLogo" src={logoUrl} alt="" /> : <span className="adminSidebarMark">T</span>}
      <span className="adminSidebarBrandText">
        {wordmarkUrl ? <img className="adminSidebarWordmark" src={wordmarkUrl} alt={siteName} /> : <strong>{siteName}</strong>}
  ...
  return <>
    <div className="adminMobileTopbar">
      <div className="adminMobileTopbarBrand">{brand}</div>
      <button
    ...
    {open ? <button className="adminSidebarOverlay" aria-label="Menüyü kapat" type="button" onClick={() => setOpen(false)} /> : null}

    <aside id="admin-sidebar-drawer" className={`adminSidebar${open ? " isOpen" : ""}`}>
      <div className="adminSidebarHeader">
        <div className="adminSidebarBrand">{brand}</div>
        <button className="adminSidebarClose" type="button" aria-label="Menüyü kapat" onClick={() => setOpen(false)}>×</button>
      </div>
      <nav className="adminSidebarNav" aria-label="Admin menüsü">
        <Link className="adminSidebarHome" href="/admin" onClick={() => setOpen(false)}><span>Genel Bakış</span><small>⌂</small></Link>
        {navGroups.map(group=><details className="adminSidebarGroup" key={group.label}><summary>{group.label}<span aria-hidden="true">⌄</span></summary><div className="adminSidebarGroupItems">{group.items.map(item=><Link className="adminSidebarItem" href={item.href} key={item.label} onClick={() => setOpen(false)}>{item.label}<span aria-hidden="true">›</span></Link>)}</div></details>)}
        <Link className="adminSidebarItem" href="/admin/guide" onClick={() => setOpen(false)}><span>Kullanım Rehberi</span><small>A-Z</small></Link>
      </nav>
