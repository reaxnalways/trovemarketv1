...
export default async function AdminPricingPage({ searchParams }: Props) {
  ...
  const [{ data: settings }, { data: products }, { data: tradeInDevices }, { data: deviceCatalog }, { data: history }, { data: faultRules }, { data: segmentRules }, { data: overrides }, { data: hybridHistory }, { data: categories }, { data: scopedHistory }] = await Promise.all([
    ...
    supabase.from("trade_in_devices").select("id,catalog_id,device_type,brand,model,storage,market_price_tr,market_price_passport,market_price_international,profit_margin_pct,is_active").order("brand").order("model").limit(1000),
    supabase.from("device_model_catalog").select("id,category,sub_category,device_type,brand,model,storage_options,color_options,reference_price_tr,price_status,is_active").eq("is_active", true).order("category").order("brand").order("model").limit(2000),
