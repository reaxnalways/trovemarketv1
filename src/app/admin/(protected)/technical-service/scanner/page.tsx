...
export default async function TechnicalServiceScannerPage({ searchParams }: Props) {
  ...
  return <main className="adminShell adminShellWide">
    ...
    {record ? <section className="listingSection"><div className="sectionHeading"><div><p className="eyebrow">BULUNAN SERVİS</p><h2>{record.first_name} {record.last_name}</h2></div><span className="productCode">{record.service_code}</span></div>
      ...
      <section className="adminDashboardCard"><p className="eyebrow">HIZLI İŞLEM</p><h2>Servis durumunu değiştir</h2>{record.archived_at ? <p>Arşivlenmiş kayıtta önce geri yükleme yap.</p> : <div className="adminInlineActions">
        {[['in_progress','Yapılmakta'],['completed','Yapıldı'],['notify_customer','Haber verilecek']].map(([value,label]) => <form action={updateTechnicalServiceStatus} key={value}><input type="hidden" name="recordId" value={record.id}/><input type="hidden" name="returnCode" value={record.barcode}/><input type="hidden" name="serviceStatus" value={value}/><button className={record.service_status === value ? "adminButton" : "adminButton adminButtonSecondary"} type="submit">{label}</button></form>)}
        <Link className="adminButton adminButtonSecondary" href={`/admin/technical-service?q=${encodeURIComponent(record.service_code)}#servis-kayitlari`}>Tam kaydı aç</Link><Link className="adminButton adminButtonSecondary" href={`/admin/technical-service/labels/${record.service_code}`}>Etiketi yazdır</Link>
      </div>}</section>
    </section> : null}
  </main>;
}
