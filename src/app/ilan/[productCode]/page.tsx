...
export default async function ListingDetailPage({ params }: ListingDetailPageProps) {
  ...
  return <><SiteHeader settings={settings} /><main className="shell listingDetailShell"><Link className="backLink" href="/">← {t.backHome}</Link><div className="listingDetailGrid"><section className="listingGallery"><ListingGallery images={listing.images} title={listing.title} /></section><section className="listingDetailCard"><span className="productCode">{listing.product_code}</span><h1 className="listingDetailTitle">{listing.title}</h1><strong className="listingDetailPrice">{formatListingPrice(listing.price)}</strong>{details.length ? <dl className="listingSpecs">{details.map(([label, value]) => <div key={String(label)}><dt>{label}</dt><dd>{value}</dd></div>)}</dl> : null}<div className="listingDetailActions">{canPurchase?<Link className="adminButton adminActionLink" href={`/satinal/${encodeURIComponent(listing.product_code)}`}>{t.buy}</Link>:null}<a className="adminButton adminButtonSecondary adminActionLink" href={buildListingWhatsAppUrl(listing.product_code, listing.title, settings.whatsapp_number)} rel="noreferrer" target="_blank">{t.moreInfo}</a><Link className="adminButton adminButtonSecondary listingTradeButton" href={tradeHref}>{t.tradeIt}</Link></div>{listing.description ? <div className="listingDescription"><h2>{t.description}</h2><p>{listing.description}</p></div> : null}</section></div></main></>;
}
