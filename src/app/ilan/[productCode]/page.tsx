...
export default async function ListingDetailPage({ params }: ListingDetailPageProps) {
  ...
  return <><SiteHeader settings={settings} /><main className="shell listingDetailShell"><Link className="backLink" href="/">← Ana sayfaya dön</Link><div className="listingDetailGrid"><section className="listingGallery"><ListingGallery images={listing.images} title={listing.title} /></section><section className="listingDetailCard"><span className="productCode">{listing.product_code}</span><h1 className="listingDetailTitle">{listing.title}</h1><strong className="listingDetailPrice">{formatListingPrice(listing.price)}</strong>{details.length ? <dl className="listingSpecs">{details.map(([label, value]) => <div key={String(label)}><dt>{label}</dt><dd>{value}</dd></div>)}</dl> : null}<div className="listingDetailActions"><a className="adminButton adminActionLink" href={buildListingWhatsAppUrl(listing.product_code, listing.title, settings.whatsapp_number)} rel="noreferrer" target="_blank">Daha Fazla Bilgi Al</a><Link className="adminButton adminButtonSecondary listingTradeButton" href={tradeHref}>Takas Et</Link></div>{listing.description ? <div className="listingDescription"><h2>Açıklama</h2><p>{listing.description}</p></div> : null}</section></div></main></>;
}
