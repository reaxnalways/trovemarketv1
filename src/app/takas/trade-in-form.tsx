...
type State = {
  ...
  buttons: string; wireless: string; cellular: string; sensors: string; liquid: string; repairCostCode: string;
...
const initialState: State = {
  ...
  buttons: "", wireless: "", cellular: "", sensors: "", liquid: "", repairCostCode: "",
...
export function TradeInForm({ whatsappNumber, devices, costOptions, targetListing }: { whatsappNumber: string; devices: TradeInCatalogDevice[]; costOptions: TradeInCostOption[]; targetListing?: TargetListing }) {
  ...
  const conditionComplete = [data.screen, data.display, data.touch, data.body, data.rear, data.battery, data.camera, data.chargingPort, data.biometric, data.speaker, data.microphone, data.buttons, data.wireless, data.cellular, data.sensors, data.liquid, data.repairCostCode, data.chargerAdapter, data.chargingCable, data.box, data.invoice, data.warranty].every(Boolean);
  ...
  async function next() {
    ...
    if (step === 2) { if (!conditionComplete) return; await calculateEstimate(); setStep(3); return; }
  ...
  function submit(event: FormEvent) {
    ...
    const message = [
      ...
      `Wi-Fi / Bluetooth: ${human(data.wireless)}`, `Şebeke / SIM: ${human(data.cellular)}`, `Sensörler: ${human(data.sensors)}`, `Sıvı teması: ${human(data.liquid)}`, `Onarım/değişen: ${optionLabel(data.repairCostCode)}`,
      "", "AKSESUAR / BELGE", `Adaptör: ${human(data.chargerAdapter)}`, `Kablo: ${human(data.chargingCable)}`, `Kutu: ${human(data.box)}`, `Fatura: ${human(data.invoice)}`, `Garanti: ${human(data.warranty)}`, data.notes ? `Not: ${data.notes}` : "",
  ...
  return <form className="tradeForm" onSubmit={submit}>
    ...
    {step === 1 ? <section className="tradeStep"><div><span className="tradeEyebrow">1 / 3</span><h2>Cihaz Bilgileri</h2></div><div className="tradeGrid">
    ...
    {step === 2 ? <section className="tradeStep"><div><span className="tradeEyebrow">2 / 3</span><h2>Detaylı Cihaz Kontrolü</h2></div><div className="tradeGrid">
    ...
    </div><div className="tradeActions"><button type="button" onClick={() => setStep(1)}>← Geri</button><button className="tradePrimary" type="button" disabled={estimating || !conditionComplete} onClick={next}>{estimating ? "Hesaplanıyor..." : "Tahmini Fiyatı Hesapla →"}</button></div></section> : null}
