...
update public.trade_in_cost_references set category='repair',selectable=true,sort_order=10 where code='previous_repair';
update public.trade_in_cost_references set category='accessory',selectable=true,sort_order=10 where code='missing_accessories';

insert into public.trade_in_cost_references(code,label,amount,category,selectable,sort_order) values
...
drop function if exists public.estimate_trade_in(uuid,text,text,text,text,text,text,text,text);
create function public.estimate_trade_in(
 p_device_id uuid,p_region text,p_cosmetic text,p_working text,p_screen text default '',p_body text default '',p_battery text default '',p_repair_cost_code text default '',p_accessory_cost_code text default ''
)
...
as $$
declare
 d public.trade_in_devices%rowtype;v_market numeric;v_margin numeric;v_deductions numeric:=0;v_battery numeric;v_raw numeric;v_costs jsonb;v_region text:=lower(trim(coalesce(p_region,'')));v_selected numeric;
begin
 select * into d from public.trade_in_devices where id=p_device_id and is_active=true;
 if not found then return; end if;
 select coalesce(jsonb_object_agg(code,amount),'{}'::jsonb) into v_costs from public.trade_in_cost_references where is_active=true;
 ...
 v_deductions:=v_deductions+case lower(trim(coalesce(p_screen,''))) when 'çatlak / kırık' then coalesce((v_costs->>'screen_replacement')::numeric,0) when 'derin çizik' then coalesce((v_costs->>'screen_deep_scratch')::numeric,0) else 0 end;
 v_deductions:=v_deductions+case lower(trim(coalesce(p_body,''))) when 'hafif izler' then coalesce((v_costs->>'body_marks')::numeric,0) when 'çizik / ezik' then coalesce((v_costs->>'body_damage')::numeric,0) when 'hasarlı' then coalesce((v_costs->>'body_heavy_damage')::numeric,0) else 0 end;
 v_deductions:=v_deductions+case lower(trim(coalesce(p_cosmetic,''))) when 'iyi' then coalesce((v_costs->>'cosmetic_good')::numeric,0) when 'orta' then coalesce((v_costs->>'cosmetic_medium')::numeric,0) when 'yıpranmış' then coalesce((v_costs->>'cosmetic_worn')::numeric,0) else 0 end;
 begin v_battery:=nullif(regexp_replace(coalesce(p_battery,''),'[^0-9.]','','g'),'')::numeric; exception when others then v_battery:=null; end;
 if v_battery is not null and v_battery<80 then v_deductions:=v_deductions+coalesce((v_costs->>'battery_replacement')::numeric,0); end if;
 v_deductions:=v_deductions+case lower(trim(coalesce(p_working,''))) when 'kısmi arızalı' then coalesce((v_costs->>'partial_fault')::numeric,0) when 'çalışmıyor' then coalesce((v_costs->>'not_working')::numeric,0) else 0 end;
 ...
 v_deductions:=v_deductions+coalesce(v_selected,0);v_selected:=null;
 select amount into v_selected from public.trade_in_cost_references where code=p_accessory_cost_code and is_active=true and selectable=true and category='accessory';
...
alter table public.service_price_references add column if not exists brand text not null default '',add column if not exists model text not null default '';
alter table public.service_price_references drop constraint if exists service_price_references_device_type_fault_code_key;
create unique index if not exists service_price_references_scope_fault_uidx on public.service_price_references(device_type,brand,model,fault_code);
...
as $$
 with candidates as (
  select r.*,case when lower(r.brand)=lower(coalesce(p_brand,'')) and lower(r.model)=lower(coalesce(p_model,'')) then 3 when lower(r.brand)=lower(coalesce(p_brand,'')) and r.model='' then 2 when r.brand='' and r.model='' then 1 else 0 end specificity
  ...
  select c.*,row_number() over(partition by c.fault_code order by c.specificity desc,c.updated_at desc) rn from candidates c where c.specificity>0
 )
